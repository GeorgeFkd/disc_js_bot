...
module.exports = {
    ...
    requiredRole: "",
    description:
        "a message is sent to you and you have to guess who deleted it",
    async execute(message, args) {
        const bot = args.pop();
        message.channel.send(
            "I see you are in for a game of snipe guesser.\nI will send you a snipe and you will have to guess the username of the sender"
        );
        ...
        const randSnipe =
            randsnipesArray[Math.floor(Math.random() * randsnipesArray.length)];
        ...
        const themsg = message.channel.send(
            `The message is ${randSnipe}.Who sent it?`
        );
        ...
        message.channel
            ...
            .then((collected) => {
                ...
                if (nameguess === username) {
                    message.channel.send(
                        `Indeed it was ${username}\'s message(so embarassing to send such stuff).\nCongrats`
                    );
                } else {
                    message.channel.send(
                        `It wasn\'t sent by ${nameguess} and i guess you will never know who sent it `
                    );
                }
            })
            .catch((collected) =>
                message.channel.send(`After a minute, you didnt guess.`)
            );
    },
};
