...
module.exports = {
    ...
    execute(message, args) {
        ...
        const filter = (reaction, user) => {
            return (
                rock_paper_scissors.includes(reaction.emoji.name) &&
                user.id === enemyID
            );
        };
        ...
        message
            .awaitReactions(filter, { max: 1, time: 8 * 1000 })
            .then((collected) => {
                const reaction = collected.first();
                console.log(reaction);
                const botChoice = Math.floor(Math.random() * 3);
                console.log(botChoice);
                const enemyChoice = rock_paper_scissors.indexOf(
                    reaction.emoji.name
                );
                if (botChoice + (1 % 3) === enemyChoice) {
                    ...
                    message.channel.send(
                        "a worthy battle that ended with a tie"
                    );
                    outcome = "tie";
                }
