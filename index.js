...
//Specific number for intents https://discord-intents-calculator.vercel.app/
...
for (const file of commandFiles) {
    ...
    // cooldown:number,requiredChannels:string[],
    // requiredRole:string,execute(msg,args)=>void}
...
bot.on("message", async (message) => {
        ...
        if (time_stamps.has(message.author.id)) {
            ...
            if (curr_time < expiration_time) {
                ...
                return message.reply(
                    `Perimene ligo re ${time_left.toFixed(
                        1
...

    {
        ...
        } else if (!theRoles.includes(command.requiredRole)) {
            //if requiredRole = '' it is good and if theRoles is ['']
            return message.reply(
                `You need the ${command.requiredRole} role to access the ${command.name} command`
            );
        }
    }
